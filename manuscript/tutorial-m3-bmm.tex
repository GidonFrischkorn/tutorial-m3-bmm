...
\shorttitle{Fitting the Memory Measurement Model (M3) with the bmm R
Package: A Tutorial}
...
Frischkorn}{0000-0002-9555-0261}} 
...
response categories that may arise from different latent processes. A
long modeling tradition argues that specific error types are
diagnostically informative because different cognitive processes imply
different conditional response probabilities over observable outcomes.
Multinomial processing tree (MPT) models formalize this logic by
treating categorical responses---including distinct error classes---as
consequences of latent states such as successful retrieval, guessing, or
source confusion (\citeproc{ref-batchelder1999theoretical}{Batchelder \&
Riefer, 1999}; \citeproc{ref-erdfelder2009multinomial}{Erdfelder et al.,
2009}). Similarly, process-dissociation approaches demonstrate that
patterns of correct and incorrect responses across task constraints can
separate the contributions of multiple processes within a single task
(\citeproc{ref-jacoby1991process}{Jacoby, 1991};
\citeproc{ref-yonelinas2012process}{Yonelinas \& Jacoby, 2012}). Across
these perspectives, the central insight is that the full multinomial
response distribution --- not accuracy alone --- provides critical
information about the latent cognitive mechanisms generating observed
behavior.
...
introduced for working memory tasks --- decomposing performance into
item activation and context-specific binding strength --- this
...
driving competitive selection, the framework applies.
...
custom JAGS or Stan code, increasing the risk of implementation errors,
limiting reproducibility, and posing a barrier to broader adoption and
extension. The \texttt{bmm} R package (\citeproc{ref-bmm2025}{Popov et
al., 2025}) addresses this limitation by providing a standardized
workflow to specify, fit, and evaluate Bayesian hierarchical M3 with
minimal code, while remaining flexible enough to accommodate new task
designs and custom model specifications. In this tutorial, we illustrate
this approach with three worked examples covering the standard simple
span (ss) and complex span (cs) tasks, and a custom extension,
demonstrating how principled modeling of categorical response data can
clarify the latent processes underlying working memory performance.
...
\tightlist
\item
  \(b\): \textbf{baseline activation}, reflecting the tendency to select
  any response option in the absence of memory-based evidence. This
  parameter is fixed (not estimated) to set the scale of the model.
\item
  \(a\): \textbf{item activation}, reflecting general memory strength
  for studied items --- the item was encountered during study,
  regardless of its position or context.
\item
  \(c\): \textbf{context binding}, reflecting the strength of the
  association between an item and its specific study context (e.g.,
  serial position, paired associate).
\end{itemize}
...
softmax rule over the simple rule for working memory data Li et al.
(\citeproc{ref-li2026filtering}{in press}). Tutorial 1 compares both
rules; Tutorials 2 and 3 use softmax as the default.
...
\item
  ...
  parameter \(f \in [0, 1]\) captures distractor filtering efficiency:
  \(f = 0\) indicates perfect filtering (distractors receive only
  baseline activation), while \(f = 1\) indicates no filtering
  (distractors are activated as strongly as memoranda).
\item
